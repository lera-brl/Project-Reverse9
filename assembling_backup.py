...
import numpy as np  # Add at the top if not present
...
def fit_to_canvas(img, canvas_width=1080, canvas_height=1920):
...
def run_emotion_session():
    ...
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1080)  #before 1080
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1920) #before 1920

    frame_width = 1080
    frame_height = 1920
    center_x = frame_width // 2
    center_y = frame_height // 2
    axes_length = (190, 270)  # горизонтальный и вертикальный радиусы
    ...
    while cap.isOpened():
        ...
        #frame = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE) #rotation - new line
        ...
        if results.multi_face_landmarks:
            ...
            if countdown_started and not photo_taken:
                ...
                if seconds_passed < timer_duration:
                    ...
                    # Если эмоция не распознана — выбрать случайную категорию
                    if category_num == 0:
                    ...
                    if source_img is not None and target_img is not None:
                        ...
                        if len(source_faces) > 0 and len(target_faces) > 0:
                            ...
                            swapped_img = fit_to_canvas(swapped_img)
                            ...
                            cv2.imwrite("output_swapped.png", swapped_img)
                            cv2.imshow("Reverse9 Result", swapped_img)
                            photo_taken = True
                        else:
